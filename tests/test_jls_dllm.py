...
from jls_backend.spectrum import decorate


class GateStaysSparseTests(unittest.TestCase):
    def test_clean_answer_does_not_activate(self):
        clean = [
            "The JLS Control Room phone is ext. 3570. [Source: facility_manual.pdf, page 4]",
...
class GateActivatesOnArtifactsTests(unittest.TestCase):
    ...
    def test_duplicate_sentences_activate(self):
        dups = [
            "The shutter must be closed first.",
            "The shutter must be closed first.",
        ]
        activate, reason = needs_correction(dups)
        self.assertTrue(activate)
...
class CorrectionValidationTests(unittest.TestCase):
    ORIGINAL = [
        "The control room is ext. 3570.",
        "Contact the floor coordinator at ext. 3639.",
    ]

    def test_valid_correction_passes(self):
        # Cleans up whitespace/punctuation while keeping every number.
        good = [
            "The control room phone is ext. 3570.",
            "Contact the floor coordinator at ext. 3639.",
        ]
        self.assertTrue(validate_correction(good, self.ORIGINAL))

    def test_dropping_a_number_fails(self):
        # Dropping ext. 3639 must fail validation.
        bad = [
            "The control room phone is ext. 3570.",
            "Contact the floor coordinator.",
        ]
        self.assertFalse(validate_correction(bad, self.ORIGINAL))

    def test_inventing_a_number_fails(self):
        # Adding an ext that wasn't in the original must fail.
        bad = [
            "The control room phone is ext. 3570.",
            "Contact the floor coordinator at ext. 3639 or cell 306-555-0199.",
        ]
        self.assertFalse(validate_correction(bad, self.ORIGINAL))

    def test_empty_correction_fails(self):
        self.assertFalse(validate_correction([], self.ORIGINAL))


class RelationDriftTests(unittest.TestCase):
    EVIDENCE = [
        "The JLS Control Room can be reached at ext. 3570. [Source: m.pdf, page 4]",
        "For hutch access contact the floor coordinator at ext. 3639. [Source: m.pdf, page 2]",
    ]

    def _is_trustworthy(self, text: str, evidence: list[str]) -> bool:
        return numbers_grounded(text, evidence) and not relation_drift(text, evidence)
    ...
    def test_elevator_drift_is_flagged(self):
        ...
        self.assertFalse(self._is_trustworthy(drifted, self.EVIDENCE))

    def test_faithful_rephrase_is_clean(self):
        faithful = "The JLS control room is reached at ext. 3570; the floor coordinator handles hutch access at ext. 3639."
        self.assertEqual(relation_drift(faithful, self.EVIDENCE), [])
        self.assertTrue(self._is_trustworthy(faithful, self.EVIDENCE))
    ...
    def test_invented_number_still_caught_by_number_guard(self):
        ...
        self.assertFalse(self._is_trustworthy(invented, self.EVIDENCE))
...
class QueryHighlightTests(unittest.TestCase):
    ...
    def test_stopwords_not_highlighted(self):
        out = decorate("The gap of the undulator.", "specs", query="the of")
...
class ExtractiveAnswerTests(unittest.TestCase):
    ROWS = [
        {
            "score": 0.85,
            "document": "Source: facility_manual.pdf\nSection: Contacts\nPage: 1\n\n"
                        "The control room phone is ext. 3570.",
            "metadata": {"source": "facility_manual.pdf", "page": 1},
        }
    ]
    ...
    def test_context_is_numbered_and_labelled(self):
        ...
        self.assertIn("[1]", out)
        self.assertIn("Source: facility_manual.pdf, page 1", out)

    def test_context_strips_chunk_header(self):
        out = answer_context(self.ROWS)
        self.assertNotIn("Section: Contacts\n", out)
        self.assertIn("ext. 3570", out)

    def test_user_prompt_carries_question_and_context(self):
        out = answer_user("control room phone", self.ROWS)
        self.assertIn("Question: control room phone", out)
        self.assertIn("ext. 3570", out)

    def test_grounding_guard_accepts_context_numbers(self):
        self.assertTrue(answer_numbers_grounded("Call ext. 3570 [1].", self.ROWS))

    def test_grounding_guard_flags_invented_numbers(self):
        self.assertFalse(answer_numbers_grounded("Reach the room at 306-555-0199 [1].", self.ROWS))
