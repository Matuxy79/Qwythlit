"""Service layer — the glue between every frontend and the ``jls_backend`` package.
...
generate_answer(...)        Optional: send the extractive answer to the carrier LLM for
                            prose clean-up (only when JLS_RETRIEVAL_ONLY=0).
...
from jls_backend.readers import load_document
...
from jls_backend.cag_cache import SemanticEvidenceCache
...
from jls_backend.pipeline import (
    EMBED_DIM,
    ...
    with _RESOURCE_LOCK:
        if _collection is None:
            _collection = get_chroma_client().get_or_create_collection(
                ...
                metadata={
                    "description": "JLS synchrotron research document retrieval",
    ...
    with _RESOURCE_LOCK:
        if _cache_collection is None:
            _cache_collection = get_chroma_client().get_or_create_collection(
                ...
                metadata={
                    "description": "JLS CAG semantic evidence cache",
                    "embedding": f"all-MiniLM-L6-v2_{EMBED_DIM}d",
                    "hnsw:space": "cosine",
                },
            )
...
def ingest_default_corpus(*, force: bool = False) -> dict[str, Any]:
    ...
    from jls_config import DEFAULT_DOCUMENT_DOMAIN, DEFAULT_DOCUMENTS_DIR
...
EVAL_CASES = [
    {
        "question": "What are the main JLS control room and floor coordinator contacts?",
...
def dllm_status(timeout: float = 1.0) -> dict:
    ...
    if RETRIEVAL_ONLY:
        detail = (
            "Retrieval-only mode is active (JLS_RETRIEVAL_ONLY=1): LLM synthesis, "
            "cleanup, and API completions are disabled. Fast deterministic mode."
        )
    elif not configured:
        detail = "Set JLS_DLLM_API_URL to enable the gpt-oss-120b answer."
        ...
        detail = (
            f"Carrier set to {carrier} but no API key found. Paste your key into jls.env "
            "(JLS_DLLM_API_KEY), then relaunch."
        )
    else:
        try:
...
def call_dllm_api(
    ...
    *,
    ...
    if not DEFAULT_DLLM_API_URL:
        raise RuntimeError("Inference carrier is not configured. Set JLS_DLLM_API_URL.")
    if system:
