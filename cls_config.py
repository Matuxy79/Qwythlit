...
without touching source code.  Set values in ``js.env`` (gitignored) before
launching; the launch scripts source that file automatically.

Key environment variables
-------------------------
JS_RETRIEVAL_ONLY       1 = disable the generative carrier entirely (default: 1)
JS_KEYWORD_ONLY         1 = skip vector search, use lexical term overlap only (default: 1)
JS_DLLM_API_KEY         API key for the generative carrier (OpenRouter)
JS_DLLM_API_URL         Base URL for the carrier (default: https://openrouter.ai/api/v1)
JS_DLLM_MODEL           Model name passed to the carrier (default: openai/gpt-oss-120b)
JS_API_URL              Internal FastAPI bridge URL (default: http://127.0.0.1:8010)
JS_CHROMA_DIR           On-disk Chroma persistent path (default: <app>/chroma_store)
JS_BOOTSTRAP_CORPUS     1 = index DEFAULT_DOCUMENTS_DIR when the store is empty
...
DEFAULT_DOCUMENTS_DIR = Path(
    os.getenv("JS_DEFAULT_DOCUMENTS_DIR", str(MANUAL_DIR / "test_books"))
)
DEFAULT_DOCUMENT_DOMAIN = os.getenv("JS_DEFAULT_DOCUMENT_DOMAIN", "")
CHROMA_DIR = Path(os.getenv("JS_CHROMA_DIR") or (APP_ROOT / "chroma_store"))
...
DEFAULT_API_URL = os.getenv("JS_API_URL", "http://127.0.0.1:8010")

# Temporary speed-first mode: keep inference on deterministic retrieval only.
# Set JS_RETRIEVAL_ONLY=0 to restore carrier synthesis/cleanup/proxy calls.
RETRIEVAL_ONLY = _env_flag("JS_RETRIEVAL_ONLY", default=True)

# Temporary keyword-first mode: skip semantic embedding/CAG cache lookup on queries and
# rank directly by lexical term overlap. Set JS_KEYWORD_ONLY=0 to restore hybrid
# semantic+lexical retrieval.
KEYWORD_ONLY_RETRIEVAL = _env_flag("JS_KEYWORD_ONLY", default=True)

# Default generative carrier: OpenRouter + openai/gpt-oss-120b (one key, 100+ models).
# Plug-and-play — paste JS_DLLM_API_KEY into js.env (gitignored) and launch.
DEFAULT_DLLM_API_URL = os.getenv("JS_DLLM_API_URL", "https://openrouter.ai/api/v1").rstrip("/")
DEFAULT_DLLM_API_KEY = os.getenv("JS_DLLM_API_KEY", "")
DEFAULT_DLLM_MODEL = os.getenv("JS_DLLM_MODEL", "openai/gpt-oss-120b")
