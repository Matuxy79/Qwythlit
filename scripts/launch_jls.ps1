...
<#
.SYNOPSIS
    Windows launcher for the JLS RAG+CAG Streamlit prototype.

    Mirrors scripts/launch_jls.sh: create .venv if needed, install packages,
    load jls.env, pick a free port, open the browser, run Streamlit.

    Double-click scripts/launch_jls.bat, or the Desktop shortcut this script
...
function Load-EnvFile {
    $envFile = Join-Path $RootDir "jls.env"
...
function Install-DesktopShortcut {
    ...
    $batPath = Join-Path $PSScriptRoot "launch_jls.bat"
    if (-not (Test-Path $batPath)) {
        Die "launch_jls.bat is missing next to this script."
    }

    $lnkPath = Join-Path $desktop "JLS RAG+CAG.lnk"
    $shell = New-Object -ComObject WScript.Shell
    $shortcut = $shell.CreateShortcut($lnkPath)
    $shortcut.TargetPath = $batPath
    $shortcut.WorkingDirectory = $RootDir
    $shortcut.WindowStyle = 1
    $shortcut.Description = "Launch the JLS RAG+CAG Streamlit prototype"
...
if ((Get-Flag "JLS_USE_API" "0") -eq "1") {
    $apiUrl = Get-Flag "JLS_API_URL" "http://127.0.0.1:8010"
    Say "API bridge enabled via JLS_API_URL=$apiUrl"
}
