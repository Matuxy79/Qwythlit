...
_LOG = logging.getLogger("cls.bootstrap")
...
def should_bootstrap_corpus() -> bool:
    ...
    value = os.getenv("CLS_BOOTSTRAP_CORPUS")
...
def _bootstrap_worker() -> None:
    try:
        ...
        from cls_service import ingest_default_corpus, service_status
...
def start_corpus_bootstrap() -> bool:
    ...
    with _START_LOCK:
        if _bootstrap_thread is not None:
            return False
        worker = threading.Thread(target=_bootstrap_worker, daemon=True, name="cls-bootstrap")
        worker.start()
